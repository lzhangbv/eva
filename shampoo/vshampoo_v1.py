...
"""Pytorch implementation of vShampoo + Grafting."""
...
class Preconditioner:
  ...
  def __init__(self, var, hps):
    ...
    # marge small dimensions
    #self._transformed_shape = _merge_small_dims(self._original_shape, hps.block_size)
    # reshape into a matrix
    #self._transformed_shape = (self._original_shape[0], int(np.prod(self._original_shape[1:])))
    ...
    if rank <= 1: # skip vectors
        ...
        self.statistics = [eps * torch.ones(s, device=device) for s in self._transformed_shape]
  ...
  def add_statistics(self, grad):
    ...
    grad = torch.reshape(grad, self._transformed_shape)
    w1 = self._hps.beta2
  ...
  def preconditioned_grad(self, grad, damping):
    ...
    for i in range(rank):
        ...
        precond_grad = torch.tensordot(
            precond_grad, v @ v.T, [[0], [0]])
        mu *= torch.norm(v) ** 2

    precond_grad.mul_(-1/(damping+mu)).add_(grad)
    precond_grad.div_(damping)

    return torch.reshape(precond_grad, self._original_shape)
